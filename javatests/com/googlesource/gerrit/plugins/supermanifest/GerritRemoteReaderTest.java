...
public class GerritRemoteReaderTest {
  ...
  @Test
  public void sha1_outterHost_repoNameFromPath() throws Exception {
    repoManager.createRepository(Project.nameKey("gerrit/project/x"));
    Repository repository = repoManager.openRepository(Project.nameKey("gerrit/project/x"));
    RevCommit projectXTip;
    try (TestRepository<Repository> git = new TestRepository<>(repository)) {
      projectXTip = git.branch(MASTER).commit().add("x", "xxx").create();
    }
  ...
  public void readWithFileMode_readsContents() throws Exception {
    ...
    // This tries to open "gerrit/project/x" which (unfortunately) works
    RepoCommand.RemoteFile remoteFile = reader.readFileWithMode(CANONICAL_WEB_URL + "/project/x", "refs/heads/master", "x");
