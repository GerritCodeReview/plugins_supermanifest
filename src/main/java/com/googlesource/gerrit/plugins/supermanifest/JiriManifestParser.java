...
class JiriManifestParser {
  static class Work {
    ...
    // we have a corresponding project int he manifest, jiri would
    ...
    //  the jiri will pin manifest2 project to A as well.
    String projectKey;
  }
  ...
  public static JiriProjects getProjects(
    ...
    while (q.size() != 0) {
      ...
      for (JiriProjects.Project project : m.projects.getProjects()) {
        project.fillDefault();
        if (w.revisionPinned && project.Key().equals(w.projectKey)) {
