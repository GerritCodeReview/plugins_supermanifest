...
public class SuperManifestIT extends PluginDaemonTest {
  ...
  public void basicTest() throws Exception {
    ...
    assertThat(branch.file("project1").getContentType()).isEqualTo(
    ...
    assertThat(branch.file("project2").getContentType()).isEqualTo(
        "x-git/gitlink; charset=UTF-8"
    );

    // Make sure config change gets picked up.
    config = "[superproject \"" + superKey.get() + ":other\"]\n"
        + "  srcRepo = " + manifestKey.get() + "\n"
        + "  srcRef = refs/heads/srcbranch\n"
        + "  srcPath = default.xml\n";
    ...
    pushFactory.create(
        db, admin.getIdent(), allProjectRepo, "Subject", "supermanifest.config",
        config).to("refs/meta/config").assertOkStatus();
    ...
    xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
        ...
        + remoteXml
        + originXml
        + "  <project name=\"" + testRepoKeys[1].get() + "\" path=\"project3\" />\n"
        + "</manifest>\n";
    ...
    assertThat(branch.file("project3").getContentType()).isEqualTo(
        "x-git/gitlink; charset=UTF-8"
    );
  }
